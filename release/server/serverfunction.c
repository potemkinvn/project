...
void SendOpponentQuit(int opponentIndex)
{
    ...
    printf("Sent message 303 to client on sockdes %d: %s\n", player[opponentIndex].sockdes, buff);
}
...
void GetMoveAndForwardMove()
{
    ...
    if(playerYIndex == -9999){
        /// opp has quit, send this info to this player
        SendOpponentQuit(i);
        return;
    }
...
void ForwardSurrenderSignal()
{
    ...
    printf("Sent message 304 to client on sockdes %d: %s\n", player[opponentIndex].sockdes, buff);
}
